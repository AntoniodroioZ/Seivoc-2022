...
<div class="container">
    <div class="row offset-md-1">
        ...
        <figure class="">
            <figcaption  style="text-align: center;">
                Área {{ $pregunta->pregunta_id - 15 }}
            </figcaption>
            <img style="height: 25rem; margin-left: 10rem; "class="" src="{{ asset($recurso->url_imagen) }}">
        </figure>
        </center>
    </div>
    ...
    <button type="button" name="" class="btn btn-outline-dark validam3" validamo3={{$respuesta[1]}}>{{ $respuesta[0] }}</button>
    @endforeach
    </center>
    <br>
    <br>
...
<script>
    ...
    $(document).ready(function(){
        ...
        $('.validam3').click(function(){
            if ($(this).attr('validamo3') == 1) {
                @if ($pregunta->pregunta_id - 15 < 5)
                location.href="{{ URL('/MiFamiliadeCarrera/Modulo4/') }}/{{ $pregunta->pregunta_id - 14 }}";
                @else
                console.log('Finalizaste Modulo 4');
                finalizaste();
                @endif
            }
            else{
    ...
    function finalizaste(){
        swal({
        title: '¡Felicidades!',
        text: 'Antes de finalizar, ayudanos a responer este cuestionario.',
        confirmButtonText:'<a href="{{url('/MiFamiliadeCarrera/redireccionamientoFormulario')}}">¡OK!</a>',
        timer: 4000
        }).then(
          function () {},
          ...
          function (dismiss) {
            if (dismiss === 'timer') {
              ...
              window.location.replace("{{url('/MiFamiliadeCarrera/redireccionamientoFormulario')}}");
            }
